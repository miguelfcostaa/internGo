  ...
  static validateUpdateData(companyData, excludeId = null) {
    ...
    if (phone !== undefined) {
      if (!phone?.trim()) {
        ...
        errors.push(
          "Telefone deve ter formato válido (9 dígitos começando por 9)."
        );
      }
    }
    ...
    if (
      morada !== undefined &&
      morada?.trim().length > 0 &&
      morada.trim().length < 5
    ) {
  ...
  static async getCompanyStats() {
    ...
    return {
      ...
      percentageWithPhotos:
        totalCompanies > 0
          ? Math.round((companiesWithPhotos / totalCompanies) * 100)
          : 0,
    };
  }
}

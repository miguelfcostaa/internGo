...
const criarEstagio = async (req, res) => {
  ...
  try {
    ...
    if (!estagioData) {
      return res
        .status(400)
        .json({ message: "Dados do estágio são obrigatórios" });
    }
...
const createAccentInsensitiveFilter = (fieldName, searchTerms) => {
  ...
  searchTerms.forEach((term) => {
    ...
    if (fieldName === "area") {
      ...
      variations.push({
        [fieldName]: {
          $elemMatch: { $regex: new RegExp(`^${flexiblePattern}$`, "i") },
        },
      });
    } else {
      ...
      variations.push({
        [fieldName]: { $regex: new RegExp(`^${escapedTerm}$`, "i") },
      });

      // 2. Busca pelo termo normalizado (sem acentos)
      const normalized = term.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
      if (normalized !== term) {
        const escapedNormalized = normalized.replace(
          /[.*+?^${}()|[\]\\]/g,
          "\\$&"
        );
        variations.push({
          [fieldName]: { $regex: new RegExp(`^${escapedNormalized}$`, "i") },
        });
      }
      ...
      variations.push({
        [fieldName]: { $regex: new RegExp(`^${flexiblePattern}$`, "i") },
      });
    }
    ...
    if (fieldName === "localizacao" && !term.includes(",")) {
      const escapedTerm = term.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      variations.push({
        [fieldName]: { $regex: new RegExp(`^${escapedTerm}\\b`, "i") },
      });
      ...
      if (normalized !== term) {
        ...
        variations.push({
          [fieldName]: { $regex: new RegExp(`^${escapedNormalized}\\b`, "i") },
        });
      }
    }
  });
...
const obterEstagioById = async (req, res) => {
  try {
    const estagio = await Estagio.findById(req.params.id).populate(
      "company",
...
const obterEstagios = async (req, res) => {
  try {
    ...
    if (req.query.area) {
      const areas = Array.isArray(req.query.area)
        ? req.query.area
        : req.query.area.split(",");
      andConditions.push(createAccentInsensitiveFilter("area", areas));
    }
    if (req.query.localizacao) {
      const localizacoes = Array.isArray(req.query.localizacao)
        ? req.query.localizacao
        : req.query.localizacao.split(",");
      andConditions.push(
        createAccentInsensitiveFilter("localizacao", localizacoes)
      );
    }
    if (req.query.duracao) {
      const duracoes = Array.isArray(req.query.duracao)
        ? req.query.duracao
        : req.query.duracao.split(",");
      filtros.duracao = { $in: duracoes.map((d) => parseInt(d)) };
    }
    if (req.query.tipoEstagio) {
      const tipos = Array.isArray(req.query.tipoEstagio)
        ? req.query.tipoEstagio
        : req.query.tipoEstagio.split(",");
      andConditions.push(createAccentInsensitiveFilter("tipoEstagio", tipos));
    }

    const finalFilter =
      andConditions.length > 1 ? { $and: andConditions } : filtros;
    const estagios = await Estagio.find(finalFilter).populate(
      "company",
      "name profilePhoto"
    );
    res.json(estagios);
...
const obterEstagiosPorEmpresa = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao buscar estágios da empresa", error });
  }
};
...
const contarEstagios = async (req, res) => {
  try {
    const nEstagios = await Estagio.countDocuments({
...
const alterarEstadoEstagio = async (req, res) => {
  try {
    ...
    estagio.status =
      req.body.status || (estagio.status === "Ativo" ? "Inativo" : "Ativo");
    await estagio.save();
    res.json(estagio);
  } catch (error) {
    res
      .status(500)
      .json({ message: "Erro ao alterar estado do estágio", error });
  }
};
...
const atualizarEstagio = async (req, res) => {
  ...
  try {
    ...
    const estagioCompanyId = estagio.company
      ? estagio.company.toString()
      : null;
  ...
  groupedValues.forEach((versions, normalized) => {
    if (isLocation) {
      ...
      if (withComma.length > 0) {
        ...
        result.push(
          withComma.reduce((longest, current) =>
        ...
        result.push(
          withoutComma.reduce((longest, current) =>
            current.length > longest.length ? current : longest
          )
        );
      }
...
const obterOpcoesFiltros = async (req, res) => {
  try {
    ...
    const localizacoes = getUniqueNormalizedValues(
      estagiosAtivos.map((estagio) => estagio.localizacao),
      true
    );
    const duracoes = [
      ...new Set(estagiosAtivos.map((estagio) => estagio.duracao)),
    ]
      .filter(Boolean)
      .sort((a, b) => a - b);
    const tiposEstagio = getUniqueNormalizedValues(
      estagiosAtivos.map((estagio) => estagio.tipoEstagio)
    );
...
const buscarEstagios = async (req, res) => {
  try {
    ...
    const estagiosUnicos = todosEstagios.filter(
      (estagio, index, self) =>
        index ===
        self.findIndex((e) => e._id.toString() === estagio._id.toString())
    );
    ...
    res
      .status(500)
      .json({ message: "Erro ao buscar estágios", error: error.message });
  }
};
...
const calcularSimilaridadeCursos = (cursoUsuario, cursosEstagio) => {
  ...
  if (Array.isArray(cursosEstagio)) {
    const cursoEncontrado = cursosEstagio.some(
      (curso) => curso.toLowerCase().trim() === cursoUsuarioLower
    );
...
const extrairCodigoPostalDaLocalizacao = (localizacao) => {
  ...
  for (const [localidade, codigo] of Object.entries(
    codigosPostaisPorLocalidade
  )) {
    if (
      localizacaoLower.includes(localidade) ||
      localidade.includes(localizacaoLower)
    ) {
      return codigo;
    }
  }
...
const calcularPontuacaoRecomendacao = (usuario, estagio) => {
  ...
  if (usuario.curso && estagio.cursosPreferenciais) {
    ...
    const similaridadeCurso = calcularSimilaridadeCursos(
      cursoUsuario,
      cursosEstagio
    );
  ...
  if (usuario.formacaoAcademica && estagio.habilitacoesMinimas) {
    ...
    const similaridadeHabilitacoes = calcularSimilaridade(
      formacaoUsuario,
      habilitacoesEstagio
    );
  ...
  if (
    ...
    const competenciasEstagio = Array.isArray(estagio.competenciasTecnicas)
      ...
      : estagio.competenciasTecnicas;
    const similaridadeCompetencias = calcularSimilaridade(
      competenciasUsuario,
      competenciasEstagio
    );
  ...
  if (
    estagio.tipoEstagio &&
    estagio.tipoEstagio.toLowerCase().includes("remoto")
  ) {
    pontuacao += 5;
  }
...
const obterEstagiosRecomendados = async (req, res) => {
  try {
    ...
    const idsEstagiosCandidatados = candidaturas.map((c) =>
      c.estagio.toString()
    );
    ...
    res.status(500).json({
      message: "Erro ao obter estágios recomendados",
      error: error.message,
    });
  }
};

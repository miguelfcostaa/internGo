...
const getCandidaturaById = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Erro ao buscar candidatura', error: error.message });
    }
};
...
const getCandidaturasByCompany = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Erro ao buscar candidaturas da empresa', error: error.message });
    }
};
...
const getCandidaturasByUser = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Erro ao buscar candidaturas do usuário', error: error.message });
    }
};
...
const getCandidaturasByEstagio = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Erro ao buscar candidaturas do estágio', error: error.message });
    }
};
...
const createCandidatura = async (req, res) => {
    try {
        ...
        const candidaturaCompleta = await Candidatura
            .findById(novaCandidatura._id)
            .populate({
                path: 'estagio',
                populate: { path: 'company', select: 'name' }
            })
    ...
    try {
        ...
        const debug = {
            ...
            comparacao: {
                ...
                saoIguais: candidatura.estagio.company._id.toString() === req.user.id.toString()
            }
        };

...
async function validateCompanyInput(Company, data) {
  ...
  if (data.phone && !validatePhoneNumber(data.phone)) {
    errors.phone =
      "Número de telefone deve ter formato válido (9xxxxxxxx ou +351xxxxxxxxx).";
  } else {
...
const validateUserUpdate = async (User, data) => {
  ...
  if (
    ...
    !/^9\d{8}$/.test(data.telefone)
  ) {
    errors.telefone =
      "Número de telefone deve conter no minimo 9 digitos e ter o formato 9xxxxxxxx.";
  ...
  if (data.codigoPostal && !/^\d{4}-\d{3}$/.test(data.codigoPostal)) {
    ...
    data.codigoPostal &&
    data.codigoPostal.replace("-", "").length !== 7
  ) {
  ...
  if (data.dataNascimento && isNaN(new Date(data.dataNascimento).getTime())) {
    errors.dataNascimento =
      "Data de Nascimento deve ter um formato válido (AAAA-MM-DD).";
  } else if (
    data.dataNascimento &&
    new Date(data.dataNascimento) > new Date()
  ) {
    ...
    errors.dataNascimento =
      "Data de Nascimento não pode ser mais de 100 anos no passado.";
  }
...
async function validateCompanyUpdate(Company, data) {
  ...
  if (data.phone && data.phone.length !== 9 && !/^9\d{8}$/.test(data.phone)) {
    errors.phone =
      "Número de telefone deve conter exatamente 9 dígitos e ter o formato 9XXXXXXXX.";
  ...
  // Valida Morada
  if (
    (data.morada && data.morada.length < 3) ||
    (data.morada && data.morada.length > 100)
  ) {
    errors.morada = "Morada deve ter entre 3 e 100 caracteres.";
  }
  ...
  if (data.codigoPostal && !/^\d{4}-\d{3}$/.test(data.codigoPostal)) {
    ...
    errors.codigoPostal =
      "Código Postal deve ter exatamente 8 caracteres (incluido o hífen).";
...
function validateCriarEstagio(Estagio, data) {
  ...
  if (data.dataInicio && !/^\d{4}-\d{2}$/.test(data.dataInicio)) {
    ...
    new Date(data.dataInicio) >
      new Date(new Date().getFullYear(), new Date().getMonth() + 12, 0)
  ) {
    errors.dataInicio =
      "A data de início não pode ser mais de 12 meses no futuro.";
  }
  ...
  if (data.tipoEstagio && !tiposValidos.includes(data.tipoEstagio)) {
    errors.tipoEstagio =
      "Tipo de estágio inválido. Opções válidas: Presencial, Remoto ou Híbrido.";
  }
  ...
  if (data.prazoCandidatura && isNaN(Date.parse(data.prazoCandidatura))) {
    ...
    data.prazoCandidatura &&
    new Date(data.prazoCandidatura) < new Date()
  ) {
    errors.prazoCandidatura = "O prazo de candidatura não pode ser no passado.";
  } else if (
    data.prazoCandidatura &&
    new Date(data.prazoCandidatura) > new Date(data.dataInicio)
  ) {
    errors.prazoCandidatura =
      "O prazo de candidatura não pode ser depois da data de início do estágio.";
  }
  ...
  if (
    data.habilitacoesMinimas &&
    !["1", "2", "3", "4", "5", "6", "7", "8"].includes(data.habilitacoesMinimas)
  ) {
  ...
  if (data.cursosPreferenciais && data.cursosPreferenciais.length > 200) {
    errors.cursosPreferenciais =
      "Cursos preferenciais deve ter no máximo 200 caracteres.";
  }

  if (data.competenciasTecnicas && data.competenciasTecnicas.length > 300) {
    errors.competenciasTecnicas =
      "Competências técnicas deve ter no máximo 300 caracteres.";
  }

  if (data.competenciasPessoais && data.competenciasPessoais.length > 200) {
    errors.competenciasPessoais =
      "Competências pessoais deve ter no máximo 200 caracteres.";
  }
  ...
  if (
    data.horaInicio &&
    data.horaFim &&
    new Date(`1970-01-01T${data.horaFim}:00`) <=
    ...
    errors.horaFim =
      "A diferença entre a hora de início e a hora de fim deve ser de pelo menos 5 horas.";
  }

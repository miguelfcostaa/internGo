  ...
  const handleFileSelect = (event) => {
    ...
    if (file) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        setError("O ficheiro é muito grande. O tamanho máximo é 5MB.");
        return;
      }
  ...
  const uploadPhoto = async (file) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.message || "Erro ao fazer upload da foto");
      }
    } catch (error) {
  ...
  return (
    <div className={styles.editablePhotoContainer}>
      <div className={styles.photoDisplay}>
        <img
          src={
            currentPhoto ? `http://localhost:5000${currentPhoto}` : profilePhoto
          }
          alt="Foto de perfil"

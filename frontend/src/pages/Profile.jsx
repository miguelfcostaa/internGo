                            ...
                            <div className={styles.candidaturasRecebidas + ' shadow'} style={{ maxHeight: '100%', overflowY: 'auto' }}>
                                {candidaturasLoading ? (
                                    ...
                                    <div key={index} className={styles.candidaturaItem}>
                                            <span><Link className={styles.verCandidatura} to={`/estagiario/${candidatura.user._id}`}>{candidatura.user.name}</Link></span>
                                            <span>{candidatura.estagio.title}</span>
                                            <span><Link className={styles.verCandidatura} to={`/ver-candidatura/${candidatura._id}`}>Ver candidatura</Link></span>
                                    </div>
                                )) : (
                        ...
                        <table className="table table-hover shadow">
                            <thead>
                                <tr>
                                    ...
                                    <th style={{ backgroundColor: '#273F4F', color: 'white' }} scope="col">Duração</th>
                                    <th style={{ backgroundColor: '#273F4F', color: 'white' }} scope="col">Tipo de Estágio</th>
                                    <th style={{ backgroundColor: '#273F4F', color: 'white', width: "8%" }} scope="col"></th>
                                </tr>
                            </thead>
                            <tbody>
                                {estagiosLoading ? (
                                    ...
                                    estagiosByCompany.map((estagio, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>{estagio.tipoEstagio}</td>
                                            <td className={styles.linkIcon} style={{ paddingRight: "2rem" }}>
                                                <Link to={`/candidaturas/${estagio._id}`} style={{ textDecoration: 'none' }}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#000" class="bi bi-eye" viewBox="0 0 16 16">
                                                        <path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8M1.173 8a13 13 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5s3.879 1.168 5.168 2.457A13 13 0 0 1 14.828 8q-.086.13-.195.288c-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5s-3.879-1.168-5.168-2.457A13 13 0 0 1 1.172 8z"/>
                                                        <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5M4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0"/>
                                                    </svg>
                                                </Link>
                                                <Link to={`/estagio/${estagio._id}/editar`} style={{ textDecoration: 'none', paddingLeft: '20px' }}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="#000" className="bi bi-pen" viewBox="0 0 16 16" style={{ cursor: 'pointer' }}>

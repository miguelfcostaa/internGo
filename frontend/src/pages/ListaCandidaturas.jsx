...
import useEstagio from "../hooks/useEstagios";

function ListaCandidaturas() {
  const { id } = useParams();
  const candidaturas = useCandidaturasEstagio(id);
  const estagio = useEstagio(id);
  console.log("estagios:", estagio);
      ...
      <div className={styles.background}>
        <div className={styles.flexColumn}>
          <div className={styles.flexRow}>
            <ButtonVoltar
              style={{ position: "absolute", top: "7rem", left: "18%" }}
            />
            <h2
              style={{
                alignSelf: "center",
            ...
            >
              <span style={{ fontWeight: "500" }}>Candidaturas Recebidas</span>{" "}
              <p>{estagio?.estagio?.title} </p>
            </h2>
          </div>
          <div>
                    ...
                    candidaturas.map((candidatura, index) => (
                      ...
                      >
                        <td style={{ padding: "0.8rem" }}>
                          ...
                          >
                            {candidatura.status.charAt(0).toUpperCase() +
                              candidatura.status.slice(1)}
                          </span>
                        </td>
                        <td>
                        ...
                        <td>
                          {handleHabilitacoes(
                            candidatura.user.formacaoAcademica
                          ) || (
                            <span style={{ color: "#aaa" }}>
                              {" "}
                              Não especificado.{" "}
                        ...
                        <td style={{ maxWidth: "100px" }}>
                          {candidatura.user.competenciasTecnicas &&
                          candidatura.user.competenciasTecnicas.length > 0 ? (
                            candidatura.user.competenciasTecnicas.join(", ")
                          ) : (
                            <span style={{ color: "#aaa" }}>
                              {" "}
                              Não especificado.{" "}
